...
reddate = redis.Redis(host='localhost', port=6379, db=9)
redkey = redis.Redis(host='localhost', port=6379, db=17)
redgirl = redis.Redis(host='localhost', port=6379, db=16)
...
def redis_save(title, link, public, public_no, public_name, girlname, keywords, description, website):
    ...
    reddate.sadd(nowdate, title)
    if keywords:
        for keyword in keywords:
            redkey.sadd(keyword, title)
    redgirl.sadd(girlname, title)

    # 将数据插入 MySQL 数据库中的 girl 表
    try:
        with connection.cursor() as cursor:
            sql = f"""
                INSERT INTO girl (address, link, girlname, keywords, albumn, albumn_no, description, date, website, score, is_exist)
                VALUES ('{title}', '{link}', '{girlname}', '{','.join(keywords)}', '{public}', '{public_no}', '{description}', '{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}', '{website}', 60, 1)
                ON DUPLICATE KEY UPDATE
                link=VALUES(link),
                girlname=VALUES(girlname),
                keywords=VALUES(keywords),
                albumn=VALUES(albumn),
                albumn_no=VALUES(albumn_no),
            ...
            try:
                ...
                try:
                    _dict = {'Xmp.dc.subject': list(keywords), 
                                'Xmp.dc.title': public +' '+girlname,
                                'Xmp.dc.description': description, 
                                'Xmp.dc.creator': [girlname], 
